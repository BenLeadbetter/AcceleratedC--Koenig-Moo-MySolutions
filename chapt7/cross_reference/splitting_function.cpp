...
vector<string> word_split(const string& str)
{
	...
	while(i != str.end())
	{
		...
		if(i != str.end())
		{
			// Decapitalize the word
			string decapitalized_word;
			transform(i, j, back_inserter(decapitalized_word), to_lower_case);
			// Add the word to the rest of the found words
			ret.push_back(decapitalized_word);
		}
		i = j;
	}
	return ret;
